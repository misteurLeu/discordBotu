...
def on_member_update(before, after):
    ...
    if str(after) in streamers and after.game.url is not None and 'twitch.tv' in after.game.url:
        yield from client.send_message(streamChannel, str(after)+' est en live sur du '+str(after.game)+' '+after.game.url)
...
#Début des commandes
    ...
    if command == "!ban" and trusted and not pm: #Cette commande sert à bannir un utilisateur, and trusted veux dire que la commande est restreinte et que seuls les utilisateurs en trust peuvent utiliser la commande et and not pm veux dire que la commande n'est pas utilisable en PM.
        id_user = message.server.get_member_named(params[1]) #La variable params[1] est le premier paramètre entré par l'utilisateur, cette ligne sert à donner l'identifiant de l'utilisateur à partir du pseudo. Pour avoir l'identifiant d'un utilisateur, un serveur ou d'autre chose, vous devez activer le mode développeur
        try:
            yield from client.ban(id_user, int(params[2])) #cette ligne sert à bannir l'utilisateur grâce à la variable id_user qui représente l'identifiant de l'utilisateur à bannir
